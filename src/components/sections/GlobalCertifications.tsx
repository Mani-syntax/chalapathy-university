          ...
          {certifications.map((cert, index) => {
            return (
              ...
              >
                ...
                <div className={`absolute top-4 right-4 min-w-[80px] max-w-[220px] bg-white rounded-md flex items-center justify-center gap-3 shadow-[0_4px_12px_rgba(0,0,0,0.1)] px-3 py-2 z-10 group-hover:scale-110 group-hover:-translate-y-1 transition-all duration-500 border border-gray-100 ${cert.images.length === 1 ? 'h-16' : 'h-12'}`}>
                  {cert.images.map((img, idx) => (
                    <img 
                      key={idx}
                      src={img} 
                      alt={`${cert.name} logo ${idx + 1}`} 
                      className={`${cert.images.length > 1 ? 'h-4 max-w-[70px]' : 'h-full max-w-[200px] scale-110'} w-auto object-contain transition-transform duration-500`}
                    />
                  ))}
                </div>
